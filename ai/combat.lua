...
local conserveMove, disabledMove
...
local function calcDamage(move, attacker, defender, rng)
	...
	if disabledMove and move.name == disabledMove then
		return 0, 0
	end
	...
	local damage = floor(floor(floor(2 * attackerLevel / 5 + 2) * math.max(1, attFactor) * move.power / math.max(1, defFactor)) / 50)
			...
			if maxDmg then
				...
				if ours then
					...
					if not ret or minTurns < bestMinTurns or maxTurns < bestTurns then
						replaces = true
					elseif (ret.accuracy < 100 or ret.name == conserveMove) and maxTurns == bestTurns then
						replaces = true
					elseif (move.accuracy < 100 or move.name == conserveMove) and maxTurns == bestTurns then
						replaces = false
...
function Combat.firstPoisoned()
	return checkBit(Memory.value("battle", "our_status"), 0x8)
end
...
function Combat.setConserving(move)
	conserveMove = move
end

function Combat.setDisabled(move)
	disabledMove = move
end

function Combat.setDisableThrash(enabled)
	Combat.setDisabled(enabled and "Thrash" or nil)
end
...
function Combat.inKillRange(draw)
	...
	if ours.hp <= hpReq then
		...
		if outsped or confused or turnsToKill > 1 or ours.speed <= enemy.speed or Pokemon.info(0, "status") > 0 then
			return ours, hpReq
		end
	end
end
