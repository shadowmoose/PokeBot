...
Strategies.timeRequirements = {
	...
	brock = function()
		return 15.25 + timeForFlier() + timeForStats(true)
	end,
...
local function takeCenter(pp, startMap, entranceX, entranceY, finishX)
	...
	if pp then
		...
		if not completedCenter and Strategies.warpToCerulean then
			completedCenter = Combat.hp() + Inventory.count("potion") * 20 >= Strategies.healthForMisty(true)
		end
	end
...
strategyFunctions.leerCaterpies = function()
	...
	return strategyFunctions.debuff {{"caterpie", leerAmount}}
end

-- checkNidoranStats

strategyFunctions.completeForest = function()
	print(Data.run.encounters_forest)
	local resetLimit = Strategies.getTimeRequirement("forest") + 2
	if Strategies.resetTime(resetLimit, "escape Viridian Forest ("..Data.run.encounters_forest.." encounters)") then
		return true
	end
...
strategyFunctions.leerMetapod = function()
	return strategyFunctions.debuff {{"caterpie",9}, {"metapod",11}}
end
...
strategyFunctions.fightSandshrew = function()
	...
	return strategyFunctions.debuff {{"sandshrew", 14, forced=forced}}
end
...
strategyFunctions.fightKoffing = function(data)
	if Strategies.trainerBattle() then
		...
		local hornAttacks = Battle.pp("horn_attack")
		if opponent == "voltorb" then
			if hornAttacks < data.min + 1 then
				forced = "double_kick"
			end
		elseif opponent == "koffing" then
			if Strategies.initialize("check_leer") then
				if Combat.isDisabled("horn_attack") or hornAttacks < data.min then
					status.leerTo = hornAttacks == 0 and 20 or 25
				end
			end
			if status.leerTo then
				if not Battle.opponentDamaged() then
					forced = "tackle"
				end
				return Strategies.buffTo("leer", status.leerTo, forced)
			end
		end
		Battle.automate(forced)
	elseif status.foughtTrainer then
...
strategyFunctions.acquireCharmander = function()
	if Strategies.initialize() then
		...
		if caught then
			Bridge.chat("caught a "..Utils.capitalize(caught).." in Mt. Moon! This skips having to get Charmander here.")
			return true
		end
	end
...
strategyFunctions.depositPokemon = function()
	Strategies.chat("deposit", "is depositing our party to save time in the Hall of Fame at the end.")
...
strategyFunctions.shopFullRestores = function()
	if Strategies.initialize() then
		Control.preferredPotion = "full_restore"
