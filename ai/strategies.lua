...
local function dodgeSideways(options)
	...
	local wx, wy = px, py
	local sy = options.sy or py
	if px * left > options.sx * left then
		wx = px + 1 * left
	elseif py == sy or py == options.dodge then
...
local function completedSkillFor(data)
	...
	elseif status.sx then
		local px, py = Player.position()
		if px ~= status.sx or py ~= status.sy then
			return true
		end
...
function Strategies.healthForMisty(withYolo)
	if Control.yolo then
		withYolo = Pokemon.hasCutter()
	end
	local healAmount = 72
	local canTwoHit = stats.nidoran.attackDV >= (withYolo and 8 or 9)
	local isSpeedTie = stats.nidoran.speedDV == 12
	local outspeeds = stats.nidoran.speedDV >= (withYolo and 12 or 13)
	if (withYolo or not Data.yellow) and canTwoHit and outspeeds then
		healAmount = 46
	elseif canTwoHit and isSpeedTie then
		healAmount = 66
	elseif withYolo then
...
Strategies.functions = {
	...
	tweetSurge = function()
		Control.preferredPotion = "super_potion"
	...
	item = function(data)
		if Battle.handleWild() then
			...
			if Strategies.initialize("checked") then
				if data.item ~= "carbos" and not Inventory.contains(data.item) then
			...
			if toHeal > 0 then
				...
				if data.forced then
					...
					if toHeal > 50 then
						...
						if Control.preferredPotion == "super_potion" or toHeal > 20 then
	...
	skill = function(data)
		...
		elseif not data.dir or Player.face(data.dir) then
			if Strategies.initialize("position") then
				if data.move == "surf" then
					status.sx, status.sy = Player.position()
					print(status.sx, status.sy)
				end
			end
	...
	debuff = function(data)
		if Strategies.trainerBattle() then
			...
			local move = Data.blue and "tail_whip" or "leer"
			return Strategies.buffTo(move, defLimit, forced)
		elseif status.foughtTrainer then
			return true
		end
	end,
	...
	fightX = function(data)
		return Strategies.prepare("x_"..data.xi)
	end,
	...
	evolveInBattle = function()
		if Strategies.initialize() then
			if Data.blue then
				status.evolving = "wartortle"
			else
				status.evolving = "nidorino"
			end
		end
		if Pokemon.inParty(status.evolving) then
			Bridge.caught(status.evolving)
			return true
		end
		if Battle.isActive() then
			status.tries = 0
		...
		elseif status.tries > 3600 then
			print("Broke from evolving "..status.evolving)
			return true
		else
	...
	reportMtMoon = function()
		...
		if moonEncounters then
			...
			local caughtCutter = Pokemon.hasCutter()
	...
	skipLearning = function()
		if Strategies.trainerBattle() then
			if Memory.value("menu", "settings_row") == 8 and Memory.value("menu", "column") == 15 then
				Input.press(Memory.raw(0x1FD6) == 1 and "A" or "B")
			else
				Battle.automate()
			end
		elseif status.foughtTrainer then
	...
	learnInBattle = function(data)
		if Strategies.trainerBattle() then
			if Pokemon.moveIndex(data.move, data.poke) then
				return true
			end
			...
			if settingsRow == 8 then
				...
				if column == 5 then
					local replaceIndex = Pokemon.moveIndex(data.replace, data.poke)
	...
	learnThrash = function()
		...
		local replaceMove = Data.yellow and "tackle" or "leer"
		if strategyFunctions.learnInBattle({move="thrash",poke="nidoking",replace=replaceMove}) then
	...
	potionBeforeMisty = function(data)
		...
		local healAmount = Strategies.healthForMisty()
		...
		if not data.goldeen and Strategies.initialize("healed") then
			...
			elseif stats.nidoran.speedDV == 12 then
			...
			elseif stats.nidoran.attackDV < 10 then
	...
	catchOddish = function()
		...
		local caught = Pokemon.hasCutter()
		if Strategies.initialize() then
			if caught then
	...
	healParalysis = function(data)
		...
		data.item = Inventory.contains("paralyze_heal", "full_restore")
		if Strategies.initialize("paralyzed") then
			local message
			if data.item then
				message = "is full restoring to cure paralysis from Oddish."
			else
	...
	giveWater = function()
		if Data.blue then
			if Inventory.count("fresh_water") < 2 then
				return true
			end
		else
	...
	safariCarbos = function()
		if Strategies.initialize() then
			...
			if Data.blue then
				if stats.squirtle.speedDV > 11 then
					return true
				end
				Bridge.chat(" This Blastoise has bad speed, so we'll need to go out of our way for the extra Carbos here.")
			else
	...
	tossInSafari = function()
		...
		return Strategies.tossItem("antidote", "tm21", "tm34", "pokeball")
	end,
	...
	centerSkip = function()
		if Strategies.initialize() then
			...
			Bridge.chat("is taking the Center to recover HP/PP for Lorelei.")
		end
			...
			if not status.frames then
				...
				if Data.run.seed then
					...
					if (Data.yellow or not INTERNAL or RESET_FOR_TIME) and not Strategies.replay then
						...
						gui.cleartext()
						Utils.drawText(0, 0, "PokeBot v"..VERSION)
						Utils.drawText(0, 7, "Seed: "..Data.run.seed)
						Utils.drawText(0, 14, "Name: "..Textbox.getNamePlaintext())
						Utils.drawText(0, 21, "Reset for time: "..tostring(RESET_FOR_TIME))
						Utils.drawText(0, 28, "Time: "..Utils.elapsedTime())
						Utils.drawText(0, 35, "Frames: "..Utils.frames())
...
function Strategies.execute(data)
	...
	if strategyFunction(data) then
		...
		if Data.blue and INTERNAL and not STREAMING_MODE then
			print(data.s)
		end
		if resetting then
			return nil
