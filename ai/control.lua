...
local controlFunctions = {
	...
	disabled = function(data)
		Combat.setDisabled(data.move)
	end,

	conserve = function(data)
		Combat.setConserving(data.move)
	end,
	...
	moon1Exp = function()
		if Data.yellow then
			...
			if INSTANT_TEXT_ROUTE then
				minExp = 1850 --TODO exact
				shouldFight = {{name="zubat"}, {name="paras"}, {name="clefairy"}, {name="geodude"}}
				oneHits = true
				allowRanges = true
			else
				minExp = 2704
	...
	moon2Exp = function()
		if Data.yellow then
			...
			if INSTANT_TEXT_ROUTE then
				minExp = 2250 --TODO exact
				oneHits = withinOneKill(15)
				allowRanges = true
			else
				minExp = 3011
				local withinOne = withinOneKill(minExp)
				local assumeLevel4 = Strategies.stats.nidoran and Strategies.stats.nidoran.level4
				if withinOne or (assumeLevel4) then
					shouldFight = {{name="zubat",exp=7.67}, {name="paras"}}
					oneHits = not assumeLevel4 or not withinOne
				end
			end
		end
	end,
		...
		for __,encounter in ipairs(shouldFight) do
			if oid == Pokemon.getID(encounter.name) and (not encounter.levels or Utils.match(opponentLevel, encounter.levels)) then
				if oneHits then
					...
					if move then
						local turnsRemaining = allowRanges and move.minTurns or move.maxTurns
