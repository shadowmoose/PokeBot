...
function Paint.draw(currentMap)
	...
	if Memory.value("battle", "our_id") > 0 then
		...
		if curr_hp == 0 then
			hpStatus = "DEAD"
		elseif curr_hp <= Combat.redHP() then
			hpStatus = "RED"
		end
		if hpStatus then
	...
	local pidx = Data.blue and Pokemon.indexOf("squirtle") or Pokemon.indexOf("nidoran", "nidorino", "nidoking")
	if pidx ~= -1 then
		local att = Pokemon.index(pidx, "attack")
		local def = Pokemon.index(pidx, "defense")
		local spd = Pokemon.index(pidx, "speed")
		local scl = Pokemon.index(pidx, "special")
		drawText(RIGHT_EDGE, 0, Pokemon.getExp(pidx).."  "..att.." "..def.." "..spd.." "..scl, true)
	end

	local repelSteps = Memory.value("player", "repel")
	if repelSteps > 0 then
		drawText(0, BOTTOM_EDGE-7, "Repel: "..repelSteps)
	end
