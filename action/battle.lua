...
local function recover()
	if Control.canRecover() then
		...
		if curr_hp > 0 then
			...
			if curr_hp < max_hp then
				local first, second
				if Control.preferredPotion == "full_restore" then
					...
					if Control.preferredPotion == "super_potion" or max_hp - curr_hp > 22 then
						first = "super_potion"
						second = "potion"
					else
						first = "potion"
	...
	if Combat.isParalyzed() and not Pokemon.isOpponent("oddish") and not Control.canDie() then
